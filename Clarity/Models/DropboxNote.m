...
{
    NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:@"DropboxNote"];
    [fetchRequest setFetchLimit:1];
    [fetchRequest setSortDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"position" ascending:NO]]];
    DropboxNote *note = [[[NoteDataManager sharedNoteDataManager].managedObjectContext executeFetchRequest:fetchRequest error:nil] lastObject];
    
    NSLog (@"DropboxNote > highestPosition: %lu\n", (unsigned long)[note.position unsignedIntegerValue]);
...
- (void)saveDropboxNote:(DropboxNote *)aNote inManagedObjectContext:(NSManagedObjectContext *)managedObjectContext
{
...
- (void)saveNote:(DropboxNote *)aNote
{
    //노트 데이트 생성 > 섹션 헤더
...
{
    return [[NSUserDefaults standardUserDefaults] integerForKey:kSELECTED_DROPBOX_NOTE_INDEX];
}


- (void)setIndexOfSelectedNote:(NSInteger)index
{
    [[NSUserDefaults standardUserDefaults] setInteger:index forKey:kSELECTED_DROPBOX_NOTE_INDEX];
}
...
{
    NSDictionary *dictionary = @{ kSELECTED_DROPBOX_NOTE_INDEX : @-1 };
...
- (NSString *)getFirstLineOfStringForTitle:(NSString *)aString
{
    ...
    {
        trimmedTitleString = @"Send from iPhone";
    }
